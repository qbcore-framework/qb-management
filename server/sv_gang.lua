...
RegisterNetEvent('qb-gangmenu:server:GradeUpdate', function(data)
	...
	if Employee then
		if Employee.Functions.SetGang(Player.PlayerData.gang.name, data.degree) then
			TriggerClientEvent('QBCore:Notify', src, "Successfully promoted!", "success")
			TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source, "You have been promoted to " ..data.degree..".", "success")
		else
